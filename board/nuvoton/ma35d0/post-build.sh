...
MODULES_DIR=board/nuvoton/ma35d0/modules/5.4.181
...
if grep -Eq "^BR2_LINUX_KERNEL_MA35_5_10_VERSION=y$" ${BR2_CONFIG}; then
	MODULES_DIR=board/nuvoton/ma35d0/modules/5.10.140
...
if grep -Eq "^BR2_MA35D0_RESIZE_SD_MAX=y$" ${BR2_CONFIG}; then
	export $(grep "BR2_MA35D0_RESIZE_DISK_DRIVE=" $BR2_CONFIG | sed 's/\"//g')
	export $(grep "BR2_MA35D0_RESIZE_DISK_NUM=" $BR2_CONFIG | sed 's/\"//g')
	echo "#!/bin/sh" >> ${RESIZE_FILE}
	echo "/etc/resize.sh ${BR2_MA35D0_RESIZE_DISK_DRIVE} ${BR2_MA35D0_RESIZE_DISK_NUM}" >> ${RESIZE_FILE}
